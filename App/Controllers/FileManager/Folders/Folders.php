...
class Folders extends FolderService
{
    ...
    /**
     * This is a public function called "_updateFolder" which takes a parameter called "$pipe". 
     * It is a try-catch block that attempts to execute the code inside the try block and catches any thrown exceptions in the catch block.  
     * @param Pipes $pipe ... Pipe validation object
     */
    public function _updateFolder($pipe) {

        try {
            // The function first calls the "folderPipe" method with the "pipe" parameter to retrieve the required parameters. 
            $params = $this->folderPipe($pipe);

            // Then, it checks if the folder exists by calling the "folderService" method with the retrieved parameters and passing false as the second argument. 
            $folderExists = $this->folderService($params->user_id, $params->id, false);
            
            // If the folder does not exist, it returns a 401 error message using the "Res" class. 
            if(!$folderExists) Res::status(401)::error("Folder not found or you do not have permission to view folder");

            // Res::send($folderExists);

            // If the folder exists, it proceeds to update the folder by calling the "updateFolderPipe" method with the "pipe" parameter and the folder object.
            
            $update = (array) $this->updateFolderPipe($pipe, $folderExists);
            // The result is cast as an array and stored in the "$update" variable. The folder object is then modified using the "$update" array, and the updated folder is sent as a response using the "Res" class. 
            $updated = $folderExists->modify($update);
            Res::send($updated);


        } catch (\Throwable $th) {
            // If any exception occurs during the execution of the code, it is caught in the catch block, and a 400 status code is returned along with the thrown exception.
           Res::status(400)->throwable($th);
        }
    }
    ...
    public function _deleteFolder()
    {
        ...
        File::findAndDelete(['folder_id' => $folderID, 'and.user_id' => $this->user->_id]);
